...
class COTREssentialTrainer(COTRTrainer):
    def __init__(self, opt, model, optimizer, criterion,
                 train_loader, val_loader):
        super().__init__(opt, model, optimizer, criterion,
                         train_loader, val_loader)
    ...
    def validate_batch(self, data_pack):
        ...
        with torch.no_grad():
            ...
            E = data_pack['E'].cuda()
            F = data_pack['F'].cuda()
            K1 = data_pack['K1'].cuda()
            K2 = data_pack['K2'].cuda()
            self.optim.zero_grad()
            output = self.model(img, query, K1, K2)
            loss = torch.nn.functional.mse_loss(output['E'], E)
            loss_data = loss.data.item()
            if np.isnan(loss_data):
                print('loss is nan while validating')
            return loss_data, output
    ...
    def validate(self):
        ...
        for batch_idx, data_pack in tqdm.tqdm(
                ...
                desc='Valid iteration=%d' % self.iteration, ncols=80,
                leave=False):
            loss_data, output = self.validate_batch(data_pack)
            val_loss_list.append(loss_data)
        mean_loss = np.array(val_loss_list).mean()
        validation_data = {'val_loss': mean_loss,
                           'output': output,
    ...
    def draw_corrs(self, imgs, corrs, weights):
        inlier_col = (0, 255, 0, 150)
        outlier_col = (255, 0, 0, 150)
        imgs = utils.torch_img_to_np_img(imgs)
        out = []
        for img, corr, W in zip(imgs, corrs, weights):
            img = np.interp(img, [img.min(), img.max()], [0, 255]).astype(np.uint8)
            img = Image.fromarray(img)
            draw = ImageDraw.Draw(img, 'RGBA')
            corr *= np.array([constants.MAX_SIZE * 2, constants.MAX_SIZE, constants.MAX_SIZE * 2, constants.MAX_SIZE])
            for c, w in zip(corr, W):
                col = tuple((np.array(inlier_col) * w + np.array(outlier_col) * (1-w)).astype(np.uint8))
                # print(col)
                draw.line(c, fill=col)
            out.append(np.array(img))
        out = np.array(out) / 255.0
    ...
    def push_validation_data(self, data_pack, validation_data):
        val_loss = validation_data['val_loss']
        pred_corrs = np.concatenate([data_pack['queries'].numpy(), validation_data['output']['pred_corrs'].cpu().numpy()], axis=-1)
        weights = validation_data['output']['weights'].cpu().numpy()
        pred_corrs = self.draw_corrs(data_pack['image'], pred_corrs, weights)
    ...
    def train_batch(self, data_pack):
        ...
        if np.isnan(loss_data):
            ...
            loss.backward()
            self.push_training_data(data_pack, output['E'], E, loss, output['weights'])
        self.optim.step()

    def push_training_data(self, data_pack, pred, target, loss, weights):
        tb_datapack = tensorboard_helper.TensorboardDatapack()
        tb_datapack.set_training(True)
        tb_datapack.set_iteration(self.iteration)
        tb_datapack.add_histogram({'distribution/pred': pred})
        tb_datapack.add_histogram({'distribution/target': target})
        tb_datapack.add_histogram({'distribution/weights': weights})
        tb_datapack.add_scalar({'loss/train': loss})
        self.tb_pusher.push_to_tensorboard(tb_datapack)
